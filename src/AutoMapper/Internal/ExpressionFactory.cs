            ...
            static Expression UpdateTarget(Expression sourceExpression, Expression newTarget)
            {
                switch (sourceExpression)
                {
                    case MethodCallExpression methodCall when methodCall.Method.IsStatic:
                        return methodCall.Update(null, new[] { newTarget }.Concat(methodCall.Arguments.Skip(1)));
                    case MethodCallExpression methodCall:
                        return methodCall.Update(newTarget, methodCall.Arguments);
                    case MemberExpression memberExpression:
                        return MakeMemberAccess(newTarget, memberExpression.Member);
                }
